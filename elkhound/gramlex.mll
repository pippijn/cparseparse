...
| "include" ws* '(' ws* '"' ([^ '"']+ as f) '"' ws* ')'		{ let nextbuf = Lexing.from_channel (open_in (state.basedir ^ "/" ^ f)) in state.stack <- nextbuf :: state.stack; normal state nextbuf }
...
(* Punctuators *)
| "{"								{ if state.in_rhs then (state.brace_level <- 1; Buffer.add_char state.code '{'; verbatim state lexbuf) else TOK_LBRACE }
| "}"								{ TOK_RBRACE }
| ":"								{ TOK_COLON }
...
| [^ '{' '}']+ as code	{ Buffer.add_string state.code code; verbatim state lexbuf }
...
| [^ '(' ')']+ as code	{ Buffer.add_string state.code code; typename state lexbuf }
