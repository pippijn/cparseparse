...
let compute_dotted_productions productions =
  let dotted_prods = Array.init (Array.length productions) (fun i ->
...
let init_env grammar =
  ...
  Reachability.compute_reachable nonterms terms prods_by_lhs grammar.start_symbol;

  let dotted_prods = compute_dotted_productions prods in
...
let run_analyses grammar =
  let env = init_env grammar in

  Timing.time Derivability.compute_derivability_relation env;
