...
module Make (L : Lexerint.Interface) = struct
  ...
  let rwlShiftTerminals (lexer : Lexerint.t) (glr: tGLR) : unit =
    ...
    while Arraystack.isNotEmpty glr.prevTopmost do
  ...
  let printParseErrorMessage tokType glr lastToDie =
    if not glr.noisyFailedParse then
      ...
      Printf.printf (*loc*) "Parse error (state %d) at %s\n"
                    lastToDie
                    (L.tokenKindDesc tokType);

      flush stdout;
    end


  let nondeterministicParseToken lexer glr =
    ...
    (* do all shifts last *)
    rwlShiftTerminals lexer glr;

    (* error? *)
    if Arraystack.isEmpty glr.topmostParsers then (
      printParseErrorMessage lexer.tokType glr !lastToDie;
      false
    ) else (
      true
    )
  ...
  let glrParse glr getToken lexer treeTop =
    ...
    begin
      let open Objpool in
      let open Arraystack in
      stackNodePool.stack.arr <- growArray stackNodePool.stack.arr 512 stackNodePool.stack.null
    end;
    ...
    let glrParseToken lexer =
      if not (nondeterministicParseToken lexer glr) then
          ...
          if isReduceAction action then (
            ...
            if rhsLen <= !parsr.determinDepth then (
              ...
              if use_keep && not (userAct.keepNontermValue lhsIndex sval) then (
                printParseErrorMessage lexer.tokType glr newNode.state;
                if accounting then (
